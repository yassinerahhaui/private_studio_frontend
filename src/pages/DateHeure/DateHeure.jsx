  ...
  return (
    <div className={styles.DateHeure} id='dateHeure'>
      <div className={styles.elementsBox}>
        {appointment.coupe.map(el=> <Coupe key={el.id} data={el} />)}
      </div>
      <Link to='/#service' className={styles.button} id='new_coupe_btn'>{t('add_service')}</Link>
      ...
      <div className={styles.selectedTime} id='selected_time'>
        ...
        <button className={styles.selectedTimeButton} id='edit_btn'><MdOutlineEdit /></button>
      </div>
      <h2 className={styles.title2} id='identification'>3.{t('identification')}</h2>
      <div className={styles.formBox} style={{display: 'none'}} id='form_box'>
        <form action="" method="post">
          <label htmlFor="form_name">{t('name')}</label>
          <input type="text" name="name" onChange={(e)=> setFullname(e.target.value)} id="form_name" />
          <label htmlFor="form_email">{t('email')}</label>
          <input type="email" name="email" onChange={(e)=> setEmail(e.target.value)} id="form_email" />
          <label htmlFor="form_phone">{t('phone')}</label>
          <input type="tel" name="phone" onChange={(e)=> setPhone(e.target.value)} id="form_phone" />
          <button onClick={()=> get_appointment()} className={styles.button}>{t('submit')}</button>
        </form>
      </div>
    </div>
  );
